...
class FullBody(Robot):
    ...
    def generateStateInContact(self, configuration, direction, acceleration=[0, 0, 0], robustnessThreshold=0):
        return self.clientRbprm.rbprm.generateStateInContact(configuration, direction, acceleration,
                                                             robustnessThreshold)
    ...
    def interpolate(self,
        ...
        if (filterStates):
            ...
            filt = 0
        return self.clientRbprm.rbprm.interpolate(stepsize, pathId, robustnessTreshold, filt, testReachability,
    ...
    def computeContactForConfig(self, q, limbName):
        ...
        return [rawdata[i:i + 3]
                for i in range(0, len(rawdata), 6)], [rawdata[i + 3:i + 6] for i in range(0, len(rawdata), 6)]
    ...
    # # Provided a center of mass trajectory has already been computed and interpolated, generate a continuous full
    # body path between two indicated states. The states do not need to be consecutive, but increasing in Id.
    # Will fail if the index of the states do not exist
    ...
    def effectorRRTFromPosBetweenState(self, state1, state2, comPos1, comPos2, comPos3, numOptim=10):
        return self.clientRbprm.rbprm.effectorRRTFromPosBetweenState(state1, state2, comPos1, comPos2, comPos3,
                                                                     numOptim)
    ...
    def effectorRRTFromPath(self,
                            ...
                            numOptim=10,
                            trackedEffectors=[]):
        return self.clientRbprm.rbprm.effectorRRTFromPath(state1, refPathId, path_start, path_to, comPos1, comPos2,
                                                          comPos3, numOptim, trackedEffectors)
    ...
    # note : it depend on the implementation of interpolate : which state do we add to the list : the first one or the
    # last or one in the middle current implementation : the last one
    # \param stateId : index of the state
    def getDurationForState(self, stateId):
        if stateId == 0:
            ...
            return (self.clientRbprm.rbprm.getTimeAtState(stateId) -
                    self.clientRbprm.rbprm.getTimeAtState(stateId - 1))
    ...
    def isDynamicallyReachableFromState(self,
                                        stateFrom,
                                        stateTo,
                                        addPathPerPhase=False,
                                        timings=[],
                                        numPointsPerPhases=5):
        return self.clientRbprm.rbprm.isDynamicallyReachableFromState(stateFrom, stateTo, addPathPerPhase, timings,
                                                                      numPointsPerPhases)
